...
fn update(@builtin(global_invocation_id) invocation_id: vec3<u32>) {
    ...
    if alpha >= 0.0 {
        ...
        var fg = vec4<f32>(1.0, 1.0, 1.0, alpha);
        let blend = blend_normal(alpha, bg, fg);
        textureStore(input, location, blend);
    }
}

fn blend_normal(alpha: f32, bg: vec4<f32>, fg: vec4<f32>) -> vec4<f32> {
    return vec4<f32>(
...
fn blend_erase(alpha: f32, bg: vec4<f32>, fg: vec4<f32>) -> vec4<f32> {
    return vec4<f32>(
        bg.rgb,
        clamp(bg.a - fg.a, 0.0, 1.0)
    );
...
fn brush_alpha(
    ...
    let nearest_point = mouse_positions[0] + line_direction * clamped_projection_length;
